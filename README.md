...
- Create a copy of ``trivia_app/settings/local.py.example``:  
  `cp trivia_app/settings/local.py.example trivia_app/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/trivia_app/settings/base.py``.
- Adjust ``backend/trivia_app/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
